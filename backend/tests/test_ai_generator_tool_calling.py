...
class TestAIGeneratorToolCalling(unittest.TestCase):
    ...
    def test_generate_response_with_tool_use(self, mock_anthropic_class):
        ...
        tool_use_content = [
            MockAnthropicContentBlock("text", "I need to search for information about this topic."),
            MockAnthropicContentBlock(
                "tool_use", 
                name="search_course_content", 
                input_data={"query": "Python variables", "course_name": "Python Basics"}, 
                block_id="tool_123"
            )
        ]
        ...
        final_response = MockAnthropicResponse("Python variables are used to store data values. They are containers that hold different types of data like strings, numbers, and booleans.")
    ...
    def test_tool_execution_flow_messages(self, mock_anthropic_class):
        ...
        # Final response
        final_response = MockAnthropicResponse("Based on the course materials, machine learning is...")
    ...
    def test_multiple_tool_calls_in_response(self, mock_anthropic_class):
        ...
        tool_use_content = [
            ...
            MockAnthropicContentBlock(
                "tool_use", 
                name="search_course_content",
                input_data={"query": "Python advanced", "course_name": "Advanced Python"}, 
                block_id="tool_2"
            )
        ]
        first_response = MockAnthropicResponse(tool_use_content, stop_reason="tool_use")
        final_response = MockAnthropicResponse("Combining information from both Python courses...")
    ...
    def test_tool_execution_error_handling(self, mock_anthropic_class):
        ...
        first_response = MockAnthropicResponse(tool_use_content, stop_reason="tool_use")
        final_response = MockAnthropicResponse("I apologize, but I encountered an error while searching...")
    ...
    def test_conversation_history_with_tools(self, mock_anthropic_class):
        ...
        first_response = MockAnthropicResponse(tool_use_content, stop_reason="tool_use")
        final_response = MockAnthropicResponse("Building on our previous discussion about variables...")
        
        mock_client.messages.create.side_effect = [first_response, final_response]
        self.mock_tool_manager.execute_tool.return_value = "[Python Basics - Lesson 3]\nFunctions in Python..."
...
class TestAIGeneratorRealToolIntegration(unittest.TestCase):
    ...
    def test_real_tool_execution_flow(self, mock_anthropic_class):
        ...
        first_response = MockAnthropicResponse(tool_use_content, stop_reason="tool_use")
        final_response = MockAnthropicResponse("Database design follows several key principles...")
        
        mock_client.messages.create.side_effect = [first_response, final_response]
        
        # Mock vector store search result
        mock_search_result = SearchResults(
            documents=["Database normalization reduces redundancy and improves data integrity."],
            metadata=[{"course_title": "Database Systems", "lesson_number": 3}],
            distances=[0.15],
            error=None
        )
        self.mock_vector_store.search.return_value = mock_search_result
        self.mock_vector_store.get_lesson_link.return_value = "https://example.com/db-lesson3"
    ...
    def test_real_tool_with_empty_results_scenario(self, mock_anthropic_class):
        ...
        first_response = MockAnthropicResponse(tool_use_content, stop_reason="tool_use")
        final_response = MockAnthropicResponse("I couldn't find relevant information about that topic.")
